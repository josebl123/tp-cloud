...
public class QueueViewFactory {
    ...
    public QueueSnapshot snapshot(ServiceQueue queue, List<QueueEntry> waiting, List<QueueEntry> inService) {
        ...
        List<EntryView> waitingViews = waiting.stream()
                .map(entry -> entryView(entry, estimationService.estimate(queue, waiting, inService, entry, estimate.duration())))
                .toList();
    ...
    public TicketView ticketView(QueueEntry entry, Integer ignored, int inServiceCount, Duration ignoredAverage) {
        return ticketView(entry, null);
    }
